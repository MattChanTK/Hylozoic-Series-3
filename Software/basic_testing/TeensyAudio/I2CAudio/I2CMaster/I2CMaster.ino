...
void loop()
{
    ...
    switch(Wire.status())
    {
      case I2C_ADDR_NAK:
        ...
        while(digitalRead(sda_pin) == 0 && count++ < 100)
        {
...
void i2cHeartbeat(){
  // Every 10 seconds request a signal from the I2C Slave(s)
  // If the signal does not come back, the line may be locked up
  // In that case, send the resetBus signal
  // First use I2C status to determine where it's getting locked up
