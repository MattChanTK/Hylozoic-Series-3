			...
			
{
	...
	if (is_all_slow){
		trig_pins[0] = teensy_unit.SPWM_pin[port_id][0];
		trig_pins[1] = teensy_unit.SPWM_pin[port_id][1];
	}
	else{
		trig_pins[0] = teensy_unit.FPWM_pin[port_id][0];
		trig_pins[1] = teensy_unit.FPWM_pin[port_id][1];
	}
	

	din_pins[0] = teensy_unit.Analog_pin[port_id][0];
	din_pins[1] = teensy_unit.Analog_pin[port_id][1];
...
//~~outputs~~
void TeensyUnit::SoundPort::set_trig_out(const uint8_t id, const uint8_t state){

	if (is_all_slow){
		
		noInterrupts();
		teensy_unit.spwm.setPWMFast(trig_pins[id], 4095*(state>0));
		interrupts();
	}
	else{
		digitalWrite(trig_pins[id], state > 0);
	}
}

//~~inputs~~
uint16_t TeensyUnit::SoundPort::read_din_state(const uint8_t id){  
	return (uint16_t) analogRead(din_pins[id]);
}
